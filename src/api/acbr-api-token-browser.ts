...
/**
 * Obtém um *access token* da Acbr Api **no navegador** via OAuth2 *Client Credentials*.
 *
 ...
 *                     @defaultValue `"https://auth.acbr.api.br/realms/ACBrAPI/protocol/openid-connect/token"`
 ...
 * @example
 * ```ts
 * const token = await getAcbrApiTokenForBrowser(
 *   "CLIENT_ID",
 *   "CLIENT_SECRET"
 * );
...
export async function getAcbrApiTokenForBrowser(
  clientId: string,
  clientSecret: string,
  tokenUrl: string = "https://auth.acbr.api.br/realms/ACBrAPI/protocol/openid-connect/token",
