 ...
 *   node --loader ts-node/esm acbr-api-run-example.ts
 *
 *   # ou com ts-node via npx
 *   npx ts-node acbr-api-run-example.ts
 *
...
import { AcbrApi } from "../src/api/acbr-api";
import { getClientCredentialsToken } from "../src/api/acbr-api-token-acquire";
...
async function main() {
  ...
  const api = new AcbrApi(http, API_BASE);
